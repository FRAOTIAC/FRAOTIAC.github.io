# Swift in Eye of C++ & Python Devs · Part 1
...
这样的设计有充分的理由：在 iOS 设备上运行 App，若采用 JIT，性能和资源消耗都会成为大问题。而 AOT 编译的 Swift，既保证了 App 的流畅体验，也符合苹果一直以来对性能的追求。（当然，从商业角度来看，也有助于苹果“卖金子内存”XD）
...
1. 变量
    ...
    而`let，var`这样的类型推导标识，让我想起了CPP的auto，经常写cpp的人知道，面对auto的接受度，cpp开发群体的意见是不一致的，比如我们就有这样的一个auto meme：https://www.reddit.com/r/ProgrammerHumor/comments/sp792s/this_is_how_i_see_modern_c/，当然我自己是会倾向于写auto的，因为省事，但是在另一些情况下，最好清楚声明变量定义。
2. 函数定义
    1. 无参函数
        ...
        1. swift需要关键字func来定义函数
        2. swift 无需包含头文件，print是内建函数
    2. 有参函数
        ...
        可以看到，在函数参数这里，Swift的写法与CPP是不同的，首先是变量定义，紧跟冒号，最后是类型定义，而在调用的时候，Swift需要指定参数名称，然后才可以调用。这个设计对于cpp/python开发者来说是需要是适应的。当然你可以使用_来关闭参数名强制，也可以指定外部和内部参数名，但是作为cpp开发者 I don’t care. 
        
        cpp开发者在这里可以看出来，number，end这两个参数是值传递的，因此会经过一次拷贝，在函数内部这两个值是可以改变的。cpp也允许隐式类型转换。
    ...
    3. 返回值
        ...
        有一个很大的区别是，cpp是允许在有返回值定义的函数中，不return的（UB），而swift则不允许，这里也是安全设计上的考虑。 
        ...
        上面这个函数是否会返回计算值？如果cpp中写类似的函数，那么这个函数是不会返回计算值的，甚至会被编译优化掉这个函数。但是swift在这里会，特别的，当函数体只有一行表达式且指定了返回值，return是可以省略的。 
    ...
    4. 自定义参数标签（customize parameter labels）
        ...
        至于为什么，看过Severance的我无需解释，那些没看过的读者真的十分推荐。
        
        还记得我们之前提到过swift也支持在调用函数的时候省略参数名称吗，其实不是省略，而是用_定义这个参数的outie，这样在调用的时候，实际上你使用的是_，在语义上就是可以省略的。
    ...
    | 特性 | Swift | C++ | Python |
    | --- | --- | --- | --- |
    | 默认值支持 | ✅ `end: Int = 12` | ✅ `int end = 12` | ✅ `def f(end=12):` |
    | 参数标签语义 | ✅ 强，`for number:` | ❌ 仅位置参数 | ✅ 仅关键字参数 |
    | 是否支持省略中间参数 | ❌ 不支持，必须从右往左连续默认 | ✅ 支持跳过中间（但容易出错） | ✅ 可指定关键字，顺序不重要 |
    | 推荐写法 | 参数少时使用默认值 + 标签 | 少数参数支持默认，重载更常用 | 默认值广泛使用 |
